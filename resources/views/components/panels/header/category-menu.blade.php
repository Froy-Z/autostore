        ...
        @foreach($categories as $category)
            <li class="group">
                @if($category->children->isNotEmpty())
                    ...
                    >
                        {{ $category->name }}
                        <x-icons.chevron_down class="inline-block h-4 w-4"/>
                    </a>
                    <ul class="dropdown-navigation-submenu absolute hidden group-hover:block bg-white shadow-lg">
                        @foreach($category->children as $children)
                            <li><a class="block py-2 px-4 @if($selectedCategory($children)) text-orange @else text-black @endif hover:text-orange hover:bg-gray-100"
                                   href="{{ route('catalog', $children) }}">{{ $children->name }}</a>
                            </li>
                        @endforeach
                    </ul>
                @else
                    <a class="inline-block p-4 @if ($selectedCategory($category)) text-orange @else text-black @endif font-bold hover:text-orange"
                        href="{{ route('catalog', $category) }}"
                    >{{ $category->name }}</a>
                @endif
            </li>
